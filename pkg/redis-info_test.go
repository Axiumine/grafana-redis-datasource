...
func TestQueryInfo(t *testing.T) {
	...
	tests := []struct {
		...
		{
			"should parse bulk string with 'redis v6.2 errorstats' section",
			queryModel{Command: models.Info, Section: "errorstats"},
			"# Errorstats\r\nerrorstat_ERR:count=5\r\nerrorstat_WRONGTYPE:count=10\r\n",
			2,
			2,
			[]valueToCheckInResponse{
				{frameIndex: 0, fieldIndex: 0, rowIndex: 0, value: "ERR"},
				{frameIndex: 0, fieldIndex: 1, rowIndex: 0, value: int64(5)},
				{frameIndex: 0, fieldIndex: 0, rowIndex: 1, value: "WRONGTYPE"},
				{frameIndex: 0, fieldIndex: 1, rowIndex: 1, value: int64(10)},
			},
			nil,
		},
		{
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			response := queryInfo(tt.qm, &client)
			if tt.err != nil {
...
func TestQueryClientList(t *testing.T) {
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			client := testClient{rcv: tt.rcv, err: tt.err}
			response := queryClientList(tt.qm, &client)
			if tt.err != nil {
				require.EqualError(t, response.Error, tt.err.Error(), "Should set error to response if failed")
				require.Nil(t, response.Frames, "No frames should be created if failed")
			} else {
...
func TestQuerySlowlogGet(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			queryModel{Command: models.SlowlogGet, Size: 2},
			[]interface{}{
				[]interface{}{int64(14), int64(1309448221), int64(15), []interface{}{"ping"}, "127.0.0.1:58217", "worker-123"},
			},
			8,
		...
		{
			"should parse the argument count added in redis 8.10",
			queryModel{Command: models.SlowlogGet},
			[]interface{}{
				[]interface{}{int64(14), int64(1309448221), int64(15), []interface{}{"set", "foo", "bar"},
					"172.17.0.1:38994", "worker-123", int64(3)},
			},
			8,
			1,
			[]valueToCheckInResponse{
				{frameIndex: 0, fieldIndex: 3, rowIndex: 0, value: "set foo bar"},
				{frameIndex: 0, fieldIndex: 4, rowIndex: 0, value: "172.17.0.1:38994"},
				{frameIndex: 0, fieldIndex: 5, rowIndex: 0, value: "worker-123"},
				{frameIndex: 0, fieldIndex: 6, rowIndex: 0, value: int64Pointer(3)},
				{frameIndex: 0, fieldIndex: 7, rowIndex: 0, value: false},
			},
			nil,
		},
		{
			"should flag entries whose arguments redis truncated",
			queryModel{Command: models.SlowlogGet},
			[]interface{}{
				[]interface{}{int64(17), int64(1309448221), int64(18),
					[]interface{}{"RPUSH", "biglist", "1", "... (11 more arguments)"},
					"172.17.0.1:34078", "", int64(42)},
			},
			8,
			1,
			[]valueToCheckInResponse{
				{frameIndex: 0, fieldIndex: 6, rowIndex: 0, value: int64Pointer(42)},
				{frameIndex: 0, fieldIndex: 7, rowIndex: 0, value: true},
			},
			nil,
		},
		{
			"should leave the argument count empty on servers that do not report it",
			queryModel{Command: models.SlowlogGet},
			[]interface{}{
				[]interface{}{int64(14), int64(1309448221), int64(15), []interface{}{"ping"}, "127.0.0.1:58217", "worker-123"},
			},
			...
			1,
			[]valueToCheckInResponse{
				{frameIndex: 0, fieldIndex: 6, rowIndex: 0, value: (*int64)(nil)},
				{frameIndex: 0, fieldIndex: 7, rowIndex: 0, value: false},
			},
			nil,
		},
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			client := testClient{rcv: tt.rcv, err: tt.err}
			response := querySlowlogGet(tt.qm, &client)
			if tt.err != nil {
				require.EqualError(t, response.Error, tt.err.Error(), "Should set error to response if failed")
				require.Nil(t, response.Frames, "No frames should be created if failed")
			} else {
