...
- Added `docker-compose/signed.yml` and `yarn start:signed`, which start Grafana against an unpacked release artifact and deliberately set no `GF_PLUGINS_ALLOW_LOADING_UNSIGNED_PLUGINS`. That omission is what makes it a test of the signature rather than of the build: a signature whose `rootUrls` do not match the instance's `root_url` is recorded as `SignatureStatusInvalid`, and `UnsignedPluginAuthorizer.CanLoadPlugin` considers only `SignatureStatusUnsigned`, so the loader drops the plugin and no setting brings it back. A signature bound to the wrong URL is worse than none
...
- Added `DEVELOPMENT.md`, which documents the build, the four Compose files and the port they publish, where each step leaves its output, how `tools/package-tag.sh` packages and signs a tag, how to verify a signed artifact before it leaves the machine, and how to install one into a Grafana that is not Compose. Upstream's `README.md` describes the plugin; none of this was written down anywhere

### Changed

- **Breaking.** The plugin id changed from upstream's `redis-datasource` to `axiumine-redis-datasource`, and `executable` with it, so the six backend binaries are now named `axiumine-redis-datasource_<os>_<arch>`. grafana.com issues a signature only when the first segment of the id names the organisation behind the access policy token, and the catalogue enforces the same `^[0-9a-z]+-([0-9a-z]+-)?(app|panel|datasource)$` shape, so `redis-datasource` is signable by RedisGrafana and by nobody else. Holding on to it would have capped the fork at unsigned builds, which no one can install without editing their `grafana.ini`, and ruled out the community signature that is the only way a third party installs this plugin at all. Being a drop-in replacement for the catalogue build was the older goal and it lost. The id is what dashboards and provisioning name the plugin by — `type` in datasource YAML, `type`, `group`, `pluginId` and `__requires` in dashboard JSON — so an existing dashboard written against `redis-datasource` will not find this datasource until those are updated
- `tools/package-tag.sh` reads the plugin id out of the tag's own `src/plugin.json` rather than hardcoding it in five places, which is what lets one script keep packaging `v2.3.0` under the old id and `v3.0.0` under the new one. `.github/workflows/main.yml` already read it that way
- The three Docker Compose files publish Grafana on host port 3854 instead of 3000, and each now sets `GF_SERVER_ROOT_URL=http://localhost:3854/`. Grafana cannot see Docker's port mapping, so `root_url` would otherwise stay at its default of `http://localhost:3000/` — and `root_url` is precisely what a plugin signature's `rootUrls` are compared against, in `urlMatch()` in `pkg/plugins/manager/signature/manifest.go`, by exact match on scheme, host, port and cleaned path
- The root `docker-compose.yml` mounts the local `dist/` and allows the renamed id to load unsigned, instead of pulling `redis-datasource` from the catalogue through `GF_INSTALL_PLUGINS`. With the rename there is nothing in the catalogue for it to fetch, and the provisioned datasource names the fork's id
...
- The badge row in `README.md` still described upstream. The three catalogue badges read their version from grafana.com's API for `redis-datasource`, `redis-app` and `redis-explorer-app` — upstream's plugins, none of which is this build, and the last two of which RedisGrafana has archived — and the Go Report Card badge pointed at upstream's repository and now renders `retired`, that service having stopped grading. All four are gone, replaced by the Grafana requirement this fork actually declares, its licence, and the `ci.yml`, `integration.yml` and `qodana.yml` workflows of this repository. A sentence under the row states that the fork is not in the catalogue, so that the absence of a catalogue badge is not read as an oversight. The screenshot above the row was also loaded from upstream's `master`, and now comes from this fork's `main`
- The provisioned datasource pointed at `redis://host.docker.internal:6379`. That name is a Docker Desktop feature and does not resolve on a Linux host, so the only datasource Grafana came up with failed its health check with `dial tcp: lookup host.docker.internal on 127.0.0.11:53: server misbehaving`. It now names the Compose file's own `redis` service, which every one of the four files defines
- The bundled `src/dashboards/redis.json` carried a navigation link to `redisgrafana.github.io`, upstream's documentation site, which this fork does not publish and cannot update. It points at this repository instead; the `redis.io` link beside it is unchanged, being Redis's own documentation rather than anyone's project page
- Three tooltips and JSDoc `@see` tags cited `oss.redislabs.com`, which no longer resolves at all: the host is gone, not redirected. The TimeSeries filter and aggregation links now point at `redis.io`'s `TS.MRANGE` and `TS.RANGE` pages, and the Cypher link at the archived `RedisGraph` repository, RedisGraph having been discontinued. Two of the three were user-visible help text in the query editor
...
- A streaming query no longer takes the whole dashboard down on Grafana 13. `TimeSeriesStreaming` handed out the `CircularDataFrame` it buffers into, whose fields are `CircularVector` proxies with a read-only `length`. Grafana 13 releases the buffers of frames it has stopped rendering by assigning `values.length = 0`, and exempts streaming frames by looking for the `appendRow` a `CircularDataFrame` carries — but a panel transformation rebuilds the frame as a plain object, so the exemption is lost while the fields still point at the same proxies, and the assignment throws a `TypeError` that escapes into React. It reached every panel on the Redis 8.x dashboard that streams and transforms, which is thirty-five of sixty-seven. The frame handed out is now a detached copy backed by ordinary arrays, so the proxies never leave the class
...
- The release workflow could never sign a build. `yarn sign` was called with no `--rootUrls`, and the access policy behind this fork issues a private signature, which is bound to the instances allowed to load the plugin and is refused with `409 InvalidArgument, Field is required: rootUrls` without them. The step now reads the list from a `GRAFANA_PLUGIN_ROOT_URLS` repository variable — the list is not a secret, but it does not belong in a public workflow file either — and checks both it and `GRAFANA_ACCESS_POLICY_TOKEN` before calling the signing service, so a missing input fails with a sentence instead of an HTTP status
- `yarn lint` died before it could report anything, so the one error it was hiding only surfaced in CI. It walked `.qodana/`, which holds the uploaded report and its minified bundles, and eslint's formatter gave up with `RangeError: Invalid string length`; raising `--max-old-space-size` only moved the failure. The directory is now ignored, a full run takes two seconds, and the error underneath it is fixed: `QueryEditor.test.tsx` declared an `Array<SelectableValue>` where `@typescript-eslint/array-type` wants `SelectableValue[]`
- The backend Qodana run failed its `CRITICAL` threshold on `npm:eslint-config-prettier:0.0.0-ANY is malicious`. The Go image has no npm resolver, so it reads the frontend dependencies without versions and matches each name against every advisory ever filed for it — here `MAL-2025-6022` and four `MSC-2025` entries, all of which cover the July 2025 compromise of 8.10.1, 9.1.1, 10.1.6 and 10.1.7. The lockfile pins 10.1.8, the release that fixed it and still the latest, and the frontend run resolves that version and passes. `qodana-go.yaml` now excludes `MaliciousLibrariesLocal` on `package.json`, which is narrower than lowering the threshold and leaves the scan that can read the version in charge of it
